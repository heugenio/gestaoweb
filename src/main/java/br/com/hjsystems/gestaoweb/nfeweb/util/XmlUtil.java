...
public class XmlUtil {
	...
	public static <T> T xmlToObject(String xml, Class<T> classe) throws JAXBException {
		...
		Unmarshaller unmarshaller = context.createUnmarshaller();
                unmarshaller.unmarshal(new StreamSource(new StringReader(xml)), classe).getValue();
		return  unmarshaller.unmarshal(new StreamSource(new StringReader(xml)), classe).getValue();
            } catch(Exception e) {
                //JOptionPane.showMessageDialog(null, "XmlUtil.xmlToObject() - Erro ao tentar capturar o resultado da Consulta: "+e.getMessage());
                return null;
            }
