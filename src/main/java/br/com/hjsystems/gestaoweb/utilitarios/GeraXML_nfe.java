...
public class GeraXML_nfe {
    ...
    public String trataTexto(String txtParam){
        if (UfEmit.equals("21")) {
            setTpemis("7");  //  SVRS
//JOptionPane.showMessageDialog(null,"geraXML_fat().trataTexto() Valor de UfEmit: "+UfEmit);
        }
        if (display)
...
//                setFinnfe("1");  //(resultset.getString("finnfe"));
            ...
            String sql_fatura = "select fatu.* from nf fatu"
                    +" where "
                    +" empresa = "+empresa
                    +" and fatu.pedido = '"+pedido+"' "
                    +" and fatu.numero_nfe = 0 "
                    +" and fatu.modelonfe = '"+modelonfe+"'";
...
if (display) JOptionPane.showMessageDialog(null,"geraXML_fat.trata_fatura() - Executou sql(1) . . .");
    ...
    if (display) JOptionPane.showMessageDialog(null,"geraXML_fat.trata_fatura() - Processando o pedido: "+getPedido());
                    setCliente(rs_mvfatura.getInt("cod_cliente"));
    if (display) JOptionPane.showMessageDialog(null,"geraXML_fat.trata_fatura() - Pegou cod_cliente: "+getCliente());
                    ...
                    setBanco(rs_mvfatura.getInt("cod_banco"));
                    setTipo_cobranca(rs_mvfatura.getString("cod_tipo_doc"));
                    setnNF(pegar_prox_numNF());
                    setForma_pgto(rs_mvfatura.getInt("cod_forma_pgto"));
                    setRefNFe(rs_mvfatura.getString("refNFe"));
                    setFinnfe(rs_mvfatura.getString("fin_nfe"));
                    if (getRefNFe() != null && getRefNFe().trim().length() == 44) {  //  chave da NFe referenciada para devolucao de compras / vendas
                        nfref = new NFref(getRefNFe());
                    } else {
                        nfref = null;
                    }
...
//  incluir para devolucao                            
//                            if (
//                                    wcfop == 5202 || //  devolucao de compras 
//                                    wcfop == 6202 || //  devolucao de compras 
//                                    wcfop == 1202 || //  devolucao de vendas
//                                    wcfop == 2202 || //  devolucao de vendas
//                                    wcfop == 1411 || //  devolucao de vendas
//                                    wcfop == 2411    //  devolucao de vendas
//                                    ) {  
//                                setFinnfe("4");
//                            }
// incluir até aqui                            
...
if (display) JOptionPane.showMessageDialog(null,"geraXML_fat.trata_detIcms() - CST: "+wicms_cst+" - consu_final: "+consu_final+" - diferido: "+diferido);
    ...
    if (display) JOptionPane.showMessageDialog(null,"geraXML_fat.trata_fatura() - passou 9 . . . ");
                                ...
                                icms_vBC = BigDecimal.ZERO;
                            } else if (CST.equals("400")){
                                icmssn102 = new ICMSSN102(origem_cst, "400");
                                icms_vBC = BigDecimal.ZERO;
    ...
    /*
                            String sql_mvestoque = "update mvestoque set documento = "+getnNF()
                                    +" where data_movimento = '"+data_mvto+"' "
                                    +" and pedido = "+getPedido()
                                    +" and sequencia = "+getSequencia()
                                    +" and item_nf = "+rs_item.getInt("item");

                            try {
    //JOptionPane.showMessageDialog(null,"Comando sql_mvestoque "+sql_mvestoque);
                                int result = con_siscom.execute(sql_mvestoque);
                                if (result > 0) {
    //JOptionPane.showMessageDialog(null,"Tabela mvestoque atualizada com sucesso! ");
                                }
                            } catch(Exception e) {
                                JOptionPane.showMessageDialog(null,"Erro ao tentar atualizar a tabela mvestoque. Erro: "+e);
                            }
     *
     */
                        }
    ...
    //                    }
                    ...
                    if (CNPJ_transp != null) {
                        ...
                        else xNome_transp = xNome_transp+" - "+wcarro;
    ...
    //JOptionPane.showMessageDialog(null,"\n        metodo trata_cobr() - dVenc: "+rs_mvfatura.getString("data_vencimento"));  //suprimir
        ...
        if (display) JOptionPane.showMessageDialog(null,"geraXML_fat.trata_fatura() - Comando sql_prazo: "+sql_prazo);
                    ...
                    while (rs_prazo.next()) {
                        ...
                        dVenc = str_dVenc;
                        vDup = "" + new BigDecimal(rs_prazo.getDouble("vlr_parcela")).setScale(2);  //total_vNF.setScale(2);  //  "" + rs_mvfatura.getBigDecimal("total_vNF").setScale(2);
        if (display) 
            JOptionPane.showMessageDialog(null,"geraXML_fat.trata_fatura() - leu nf_prazo 1 . . .  com vcto: " + dVenc);
                        //if (nome_banco != null) {
...
//            if (display) 
            ...
            JOptionPane.showMessageDialog(null,"Erro ao tentar atualizar a tabela nf. Erro: "+e);
        }
        try {
            ...
            if (dias_inicial == 0) {
                String sql_update = "update nf_prazo set"
...
if (display) JOptionPane.showMessageDialog(null,"Comando sql_dest: "+sql_dest);
        ...
        if (pessoa.equals("F")){  //  pessoa fisica
            ...
            if (
                    ...
                    UfEmit.equals("28") || //  28=SE  
                    UfEmit.equals("52")    //  52=GO  
                ) {
                setIndIEDest("9");
           }
            setIndFinal("1");
...
if (display) JOptionPane.showMessageDialog(null,"GerarXML_nfe.imprimeDanfeJava() - arquivoOrigem: "+arquivoOrigem);
       ...
       if (escolha == JOptionPane.NO_OPTION) {
           ...
           UIManager.put("OptionPane.noButtonText", "Não");
            ...
            String nDup = "", dVenc = "", vDup = "";
            int parcela = 0, idx = 0;
            nDup = "";  //Biblioteca.extrair_TAG(arq_xml, "dup", "T", "nDup", 0);
            String sql_prazo = "select * from nf_prazo"
                    +" where "
                    +" empresa = "+empresa
                    +" and pedido = "+getPedido()
...
if (display) JOptionPane.showMessageDialog(null,"geraXML_fat.trata_fatura() - Comando sql_prazo: "+sql_prazo);
            try {
                ...
                while (rs_prazo.next()) {
                    ...
                    JOptionPane.showMessageDialog(null, "Erro ao ler prazos para impressao. Erro: : "+ e);
           }
            try {
